...
Take the input string, remove the last 2 characters, and then concat this result
with itself. The fitness will be the number of non-matching characters in the
resulting string. For example, desired result of "abcde" would be "abcabc", and
a string of "abcabcrrr" would have an error of 3, for 3 too many characters, and
the string "aaaaaa" would have error of 4, since it gets 2 of the characters
right.
...
The error function (given below) is a python function that takes a Push program
as input, loads the input string into a PushInterpreter, runs the program,
and scores the output by summing the two functions described above. This
processes is reapeated for each training case, and the resulting list of
...
use of boolean instructions, float instructions, or code instructions. To lower
the search space, and make things easier for evolution, we hand pick a smaller
set of atom generators.

.. literalinclude:: /../examples/tutorials/string_demo.py
   :lines: 167-179

Another hyperparameter that is changed from its default for this problemm is the
variation operators. You can find the supported variation operators in the
...
3) the intial state of the `output structure <>`_.
...
def string_char_counts_difference(s1, s2):
    """Returns the difference in character counts between the two input strings.
    """
